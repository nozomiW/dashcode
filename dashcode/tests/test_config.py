"""config 模块单测。"""

from pathlib import Path

import pytest

from dashcode.config import Config, ConfigError, ProviderConfig, load


def _write(tmp_path: Path, content: str) -> str:
    f = tmp_path / "config.yaml"
    f.write_text(content, encoding="utf-8")
    return str(f)


def test_load_valid_single(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        """
providers:
  - name: my-claude
    protocol: anthropic
    api_key: sk-test
    model: claude-sonnet-4-6
    thinking: true
""",
    )
    cfg = load(path)
    assert isinstance(cfg, Config)
    assert len(cfg.providers) == 1
    p = cfg.providers[0]
    assert isinstance(p, ProviderConfig)
    assert p.name == "my-claude"
    assert p.protocol == "anthropic"
    assert p.api_key == "sk-test"
    assert p.model == "claude-sonnet-4-6"
    assert p.thinking is True
    assert p.base_url is None


def test_load_valid_multiple(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        """
providers:
  - name: a
    protocol: anthropic
    api_key: k1
    model: m1
  - name: b
    protocol: openai
    api_key: k2
    model: m2
    base_url: https://example.com/v1
""",
    )
    cfg = load(path)
    assert len(cfg.providers) == 2
    assert cfg.providers[0].thinking is False
    assert cfg.providers[1].base_url == "https://example.com/v1"
    assert cfg.providers[1].protocol == "openai"


def test_missing_file(tmp_path: Path) -> None:
    with pytest.raises(ConfigError, match="配置文件不存在"):
        load(str(tmp_path / "nope.yaml"))


def test_missing_api_key(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        """
providers:
  - name: a
    protocol: anthropic
    model: m1
""",
    )
    with pytest.raises(ConfigError, match=r"providers\[0\]\.api_key 不能为空"):
        load(path)


def test_invalid_protocol(tmp_path: Path) -> None:
    path = _write(
        tmp_path,
        """
providers:
  - name: a
    protocol: gemini
    api_key: k
    model: m
""",
    )
    with pytest.raises(ConfigError, match="protocol 非法"):
        load(path)


def test_empty_providers(tmp_path: Path) -> None:
    path = _write(tmp_path, "providers: []\n")
    with pytest.raises(ConfigError, match="providers 列表为空"):
        load(path)


def test_invalid_yaml(tmp_path: Path) -> None:
    path = _write(tmp_path, "[unterminated")
    with pytest.raises(ConfigError, match="YAML 解析失败"):
        load(path)
