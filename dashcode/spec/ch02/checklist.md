...
## 实现完整性
- [ ] 配置加载：合法 `.dashcode/config.yaml` 能解析出 providers 列表（验证：单测 + 启动进入对话）。(AC1/F1)
- [ ] 配置校验：缺密钥/非法 protocol/文件缺失时给出可读错误并非零退出，无未捕获堆栈（验证：删字段/改 protocol/删文件分别运行 `python -m dashcode`）。(AC1/N4)
...
- [x] 多行输入：Alt+Enter 换行、Enter 提交、提交后输入框清空（验证：`tests/test_tui_input.py` 用 `pilot.press` 真实走 `_on_key` 路径覆盖 Enter 提交 / Alt+Enter 换行 / 空输入不提交）。(AC9/F9)
...
## 编译与测试
- [ ] `python -m dashcode` 能正常启动（在合法配置下进入 TUI）。
- [x] `ruff check .` 无告警。
- [x] `ruff format --check .` 通过（或本地 `ruff format .` 已统一格式）。
- [x] `pytest` 通过（`tests/test_config.py`、`tests/test_conversation.py`、`tests/test_tui_input.py`）。
- [ ] （可选）`mypy src/dashcode` 通过（启用 strict 子集亦可）。
